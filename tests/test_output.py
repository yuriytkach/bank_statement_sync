...
class TestOutput(unittest.TestCase):
    ...
    def test_write_csv_typical_data(self):
        records = [
            ...
            {"Date": "2023/01/02", "Details": "Transaction 2 with, comma", "Sum": "-50.25"},
            {"Date": "2023/01/03", "Details": 'Transaction 3 with "quotes"', "Sum": "0.00"}
        ]
        write_csv(records, self.output_file_path)

        with open(self.output_file_path, mode='r', newline='', encoding='utf-8') as csvfile:
            ...
            self.assertEqual(i + 1, len(expected_data), "Number of data rows in CSV does not match expected.")
    ...
    def test_write_csv_field_order(self):
        records = [
            {"Sum": "10.00", "Details": "Item A", "Date": "2023/02/01"}
        ]
        write_csv(records, self.output_file_path)
