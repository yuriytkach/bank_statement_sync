...
class TestProcessorPrivat(unittest.TestCase):
    ...
    def test_process_privat_typical_file(self):
        ...
        header = ["Дата", "Опис операції", "Категорія", "Валюта картки", "Сума в валюті картки", "Валюта транзакції", "Сума в валюті транзакції"]
        data_rows = [
            ["01.01.2023 10:00:00", "Payment for goods", "Products", "UAH", -100.50, "UAH", -100.50],
            ["02.01.2023 12:30:00", "Online purchase", "Shopping", "USD", -50.00, "EUR", -45.00],
            [None, "Should be skipped", "Junk", "UAH", -10.0, "UAH", -10.0], # Row with missing date
            ["03.01.2023 15:00:00", "Coffee", "", "UAH", -25.75, "UAH", -25.75] # No category
        ]
        ...
        self.assertEqual(result[0], {"Date": "2023/01/01", "Details": "Payment for goods <Products> 10:00:00", "Sum": "-100.50"})
        # Rate: abs(-50.00) / abs(-45.00) = 50.00 / 45.00 = 1.111... -> "1.11"
        self.assertEqual(result[1], {"Date": "2023/01/02", "Details": "Online purchase <Shopping> 12:30:00 (-45.00 EUR @ 1.11)", "Sum": "-50.00"})
        self.assertEqual(result[2], {"Date": "2023/01/03", "Details": "Coffee 15:00:00", "Sum": "-25.75"})
    ...
    def test_process_privat_missing_optional_columns(self):
        ...
        # Missing "Категорія"
        header = ["Дата", "Опис операції", "Валюта картки", "Сума в валюті картки", "Валюта транзакції", "Сума в валюті транзакції"]
        data_rows = [
        ...
        self.assertEqual(result[0], {"Date": "2023/01/05", "Details": "Service payment 09:00:00", "Sum": "-200.00"})

    def test_process_privat_different_currencies(self):
        filepath = os.path.join(self.TEST_FILES_DIR, "privat_currencies.xlsx")
        header = ["Дата", "Опис операції", "Категорія", "Валюта картки", "Сума в валюті картки", "Валюта транзакції", "Сума в валюті транзакції"]
        data_rows = [
            ["06.01.2023 11:00:00", "International order", "Services", "UAH", -1500.00, "USD", -40.00], # Rate 1500/40 = 37.50
            ["07.01.2023 14:20:00", "Refund", "Income", "UAH", 300.00, "USD", 8.00] # Rate abs(300)/abs(8) = 37.50
        ]
        excel_data = [self.PRIVAT_DETECTION_ROW, header] + data_rows
        create_excel_file(filepath, "Sheet1", excel_data)
        ...
        self.assertEqual(result[0], {"Date": "2023/01/06", "Details": "International order <Services> 11:00:00 (-40.00 USD @ 37.50)", "Sum": "-1500.00"})
        self.assertEqual(result[1], {"Date": "2023/01/07", "Details": "Refund <Income> 14:20:00 (8.00 USD @ 37.50)", "Sum": "300.00"})
    ...
    def test_process_privat_file_no_header(self):
        ...
        data_rows = [
            ["01.01.2023 10:00:00", "Some data", "Category", "UAH", -100.0, "UAH", -100.0]
        ]
        excel_data = [self.PRIVAT_DETECTION_ROW, incorrect_header] + data_rows
        create_excel_file(filepath, "Sheet1", excel_data)
        
        with self.assertRaises(KeyError): # Expecting KeyError due to missing column names like "Дата"
