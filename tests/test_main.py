...
class TestMainIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.unknown_input_path = os.path.join(self.TEST_FILES_DIR, "unknown_input.xlsx")
    ...
    def _create_privat_test_file(self, file_path):
        ...
        privat_header = ["Дата", "Опис операції", "Категорія", "Валюта картки", "Сума в валюті картки", "Валюта транзакції", "Сума в валюті транзакції"]
        privat_data = [
            ["01.01.2023 10:00:00", "Test Op 1", "Cat A", "UAH", -100.00, "UAH", -100.00],
    ...
    def _create_raif_test_file(self, file_path):
        ...
        raif_header = ["Дата і час здійснення операції", "Деталі операції", "Сума у валюті операції", "Валюта", "Сума у валюті рахунку", "Сума кешбеку"]
    ...
    def test_main_privat_flow(self):
        ...
        with open(expected_output_csv, mode='r', newline='', encoding='utf-8') as csvfile:
    ...
    def test_main_unknown_structure(self):
        ...
        self.assertTrue("Error detecting structure" in result.stderr or "Unknown structure" in result.stderr or "Could not determine bank structure" in result.stderr)
